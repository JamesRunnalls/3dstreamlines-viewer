...
export class StreamLines {
  ...
  _addLines() {
    ...
    for (var p = 0; p < this.noParticles; p++) {
      let line = new Line(geometry.clone(), material.clone());
      line.age = 0;
      line.maxAge =
  ...
  animate() {
    ...
    for (var i = 0; i < this.streamlines.children.length; i++) {
      ...
      if (line.age < line.maxAge - this.fadeOut) {
        ...
        line.age++;
        var _nextPosition = this._nextPosition(
          positions[(line.age - 1) * 3],
          positions[(line.age - 1) * 3 + 2],
          positions[(line.age - 1) * 3 + 1]
        );
        if (_nextPosition) {
          positions[line.age * 3] = _nextPosition.x;
          positions[line.age * 3 + 1] = _nextPosition.z;
          positions[line.age * 3 + 2] = _nextPosition.y;
